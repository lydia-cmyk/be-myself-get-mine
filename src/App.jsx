...
export default function ProjectManager() {
  ...
  const addProject = (e) => {
    ...
    if (!name) return;
    const created = { id: uidGen(), name, emoji: "📁", colorIdx: projects.length % PROJECT_COLORS.length, tasks: [] };
...
function ProjectDetail({ project, index, setProjects, updateProjectName }) {
  ...
  const addTask = (e) => {
    ...
    setProjects((prev) => {
      const next = [...prev];
      next[index] = { ...next[index], tasks: [...(next[index].tasks || []), { ...newTask, title, id: uid() }] };
      return next;
    });
